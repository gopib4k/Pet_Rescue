...
const StatusBadge: React.FC<{ status: UIFoundPetReport['status'] }> = ({ status }) => {
    let styles = '';
        ...
        <table className="min-w-full table-fixed">
          <thead>
            <tr className="border-b border-light-primary dark:border-dark-secondary/30">
              ...
              <th className="w-1/4 py-3 px-4 text-left text-xs font-semibold uppercase tracking-wider text-light-secondary dark:text-dark-neutral">User</th>
              <th className="w-1/4 py-3 px-4 text-left text-xs font-semibold uppercase tracking-wider text-light-secondary dark:text-dark-neutral">Found At Location</th>
              <th className="w-1/6 py-3 px-4 text-left text-xs font-semibold uppercase tracking-wider text-light-secondary dark:text-dark-neutral">Pet Type</th>
              <th className="w-1/6 py-3 px-4 text-left text-xs font-semibold uppercase tracking-wider text-light-secondary dark:text-dark-neutral">Breed</th>
            ...
            {filteredReports.length > 0 ? (
              filteredReports.map((rep) => (
                <tr key={rep.id} className="border-b border-light-primary dark:border-dark-secondary/20 hover:bg-light-primary/50 dark:hover:bg-dark-background">
                  <td className="py-3 px-4 text-light-text dark:text-dark-secondary truncate" title={rep.petName}>{rep.petName}</td>
                  <td className="py-3 px-4 text-light-text dark:text-dark-secondary truncate" title={rep.user}>{rep.user}</td>
                  <td className="py-3 px-4 text-light-text dark:text-dark-secondary truncate" title={rep.location}>{rep.location}</td>
                  <td className="py-3 px-4 text-light-text dark:text-dark-secondary">{rep.petType}</td>
                  <td className="py-3 px-4 text-light-text dark:text-dark-secondary truncate" title={rep.breed}>{rep.breed}</td>
              ...
              <tr><td colSpan={7} className="text-center py-4 text-light-secondary dark:text-dark-neutral">No found pet reports found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
        <div ref={modalRef} className="w-full max-w-3xl rounded-lg shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto bg-light-neutral dark:bg-dark-primary border-t-4 border-light-accent dark:border-dark-accent" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="mb-4">
            <h3 className="text-2xl font-bold text-light-text dark:text-dark-secondary">Review Found Report ID: {formData.id}</h3>
          </div>
